...
if not st.session_state['email']['template']:
    st.title('Choose an Email template from the sidebar')
    st.stop()


st.session_state.backend.email.template.load(st.session_state['email']['template'])

col1, _, col2 = st.columns([5, 1, 10])

with col1:
    st.session_state.backend.email.template.render()
...
with col2:
    ...
    st.code(st.session_state.backend.email.template.get_filled_template(), language='markdown')

    if checkbox:
        content = st.session_state.backend.email.template.get_source_file()
